...
def _bateman(t, t0, ka, ke, amp):
    ...
    # Bateman (first-order absorption + elimination), scaled by amp
    y_val = amp * ( _np.exp(-ke*dtm) - _np.exp(-ka*dtm) ) * (ka / denom)
    y[mask] = y_val
    return y

# PK defaults (fed vs fasted); ke ≈ ln2/3.2
PK_CFG_DEFAULT = {
    "ke": 0.2167,            # 1/h (t1/2 ≈ 3.2 h)
    "ka_ir_fed": 2.0,        # 1/h
    "ka_er_fed": 0.6,        # 1/h
    "tlag_er_fed": 1.8,      # h
    "ka_ir_fast": 3.0,       # 1/h (fasted)
    "ka_er_fast": 0.8,       # 1/h
    "tlag_er_fast": 0.8,     # h
    "ir_scale": 0.8,         # exposure scaling like Gaussian amps
    "er_scale": 0.6,
}
...
def pk_dose_profile(hours_from_start, dose_entry, t0_hours, fed, cfg=None):
    ...
    ke = float(cfg.get("ke", 0.2167))
    if fed:
        ka_ir = float(cfg.get("ka_ir_fed", 2.0))
        ka_er = float(cfg.get("ka_er_fed", 0.6))
        tlag  = float(cfg.get("tlag_er_fed", 1.8))
    else:
        ka_ir = float(cfg.get("ka_ir_fast", 3.0))
        ka_er = float(cfg.get("ka_er_fast", 0.8))
        tlag  = float(cfg.get("tlag_er_fast", 0.8))

    ir_amp = cfg.get("ir_scale", 0.8) * ir_mg
    er_amp = cfg.get("er_scale", 0.6) * er_mg

    ir = _bateman(hours_from_start, t0_hours,           ka_ir, ke, ir_amp)
    er = _bateman(hours_from_start, t0_hours + tlag,    ka_er, ke, er_amp)
    return ir, er, ir + er
...
def simulate_total(t_axis, doses, start_hour):
    ...
    for d in doses:
        ...
        if st.session_state.get("model_engine", "gaussian") == "pk":
...
with st.sidebar:
    ...
    st.markdown("---")
    _model_choice = st.radio("Model engine", ["Gaussian (legacy)", "PK (Bateman, 2-input)"], index=0,
                             help="Switch between the fast Gaussian toy model and a more PK-like Bateman model.")
    if _model_choice.startswith("Gaussian"):
        st.session_state["model_engine"] = "gaussian"
    else:
        st.session_state["model_engine"] = "pk"
    if st.button("⏪ Revert to legacy Gaussian"):
        st.session_state["model_engine"] = "gaussian"
        try:
            st.rerun()
        except Exception:
...
def simulator_ui():
    ...
    plt.xlabel("Hour of day"); plt.ylabel("Conc. (arb.)"); plt.title("Simulator – " + ("PK" if st.session_state.get("model_engine","gaussian")=="pk" else "Gaussian"))
...
def optimizer_ui():
    ...
    plt.xlabel("Hour of day"); plt.ylabel("Conc. (arb.)"); plt.title("Optimizer – " + ("PK" if st.session_state.get("model_engine","gaussian")=="pk" else "Gaussian"))
    plt.grid(True, linestyle="--", alpha=0.5); plt.legend(fontsize=(8 if compact else None))
